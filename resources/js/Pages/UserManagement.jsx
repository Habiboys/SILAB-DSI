...
export default function UserManagement({ users, laboratories, roles, filters = {}, flash }) {
    ...
    const debouncedSearch = useCallback(
        debounce((value) => {
            router.get(
                ...
                { search: value || undefined, role: activeRole !== 'all' ? activeRole : undefined, perPage },
    ...
    const handleRoleFilter = (role) => {
        ...
        router.get(
            route('user-management.index'),
            { search: searchTerm || undefined, role: role !== 'all' ? role : undefined, perPage },
            { preserveState: true, preserveScroll: true },
        );
    };
    ...
    const handlePerPageChange = (e) => {
        ...
        router.get(
            route('user-management.index'),
            { search: searchTerm || undefined, role: activeRole !== 'all' ? activeRole : undefined, perPage: val },
            { preserveState: true, preserveScroll: true },
        );
    };
    ...
    const roleTabs = [
        { key: 'all', label: 'Semua' },
        ...roles.map((r) => ({ key: r.name, label: ROLE_LABELS[r.name] || r.name })),
    ];
    ...
    const renderFormFields = (form, isEdit = false) => (
        <>
            ...
            <div>
                ...
                {form.errors.roles && <p className="mt-1 text-sm text-red-600">{form.errors.roles}</p>}
            </div>
            ...
            {needsLab(form.data.roles) && (
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => form.setData('laboratory_id', e.target.value)}
                    >
                    ...
                    {form.errors.laboratory_id && (
                        <p className="mt-1 text-sm text-red-600">{form.errors.laboratory_id}</p>
                    )}
                    <p className="mt-1 text-xs text-gray-500">
                        Lab ini akan ditetapkan sebagai akses langsung admin ke lab tersebut.
                    </p>
                </div>
            )}

            {/* Info text for non-admin roles */}
            {(form.data.roles.includes('asisten') || form.data.roles.includes('dosen')) &&
                !needsLab(form.data.roles) && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-xs text-blue-700">
                            <strong>Info:</strong> Asisten dan Dosen mendapatkan akses laboratorium melalui kepengurusan
                            yang aktif, bukan melalui pengaturan langsung di sini.
                        </p>
                    </div>
                )}
        </>
                                            ...
                                            {user.laboratory ? (
                                                <div>
                                                    ...
                                                    <span className="ml-1 text-xs text-gray-400">
                                                        ({user.laboratory.source === 'kepengurusan' ? 'kepengurusan' : 'langsung'})
                                                    </span>
                                                </div>
                                            ) : (
