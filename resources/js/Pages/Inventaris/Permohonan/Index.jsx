...
export default function PermohonanIndex({ permohonan, filters }) {
    const { auth, laboratorium: globalLab } = usePage().props;
    ...
    const allSelected = permohonan.data.length > 0 && selectedIds.length === permohonan.data.length;
    ...
    const executeBulkDelete = () => {
        router.post(route('inventaris.permohonan.bulk-delete'), { ids: selectedIds }, {
    ...
    const { data, setData, post, processing, errors, reset } = useForm({
        laboratorium_id: auth.user.laboratory_id || (globalLab?.id) || "", 
    ...
    const applyFilters = debounce((search, status, pageSize) => {
        router.get(route('inventaris.permohonan.index'), {
            search, status, perPage: pageSize, lab_id: filters.lab_id
    ...
    const handleSubmit = (e) => {
        ...
        post(route('inventaris.permohonan.store'), {
            ...
            onError: (err) => {
                toast.error('Gagal mengajukan permohonan, periksa kembali inputan Anda.');
                console.error(err);
            }
        });
    };
        ...
        <DashboardLayout>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <div className="p-6 flex justify-between items-center border-b">
                    <h2 className="text-xl font-semibold text-gray-800">Permohonan Aset</h2>
                    <button
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {permohonan.data.length > 0 ? permohonan.data.map((item, index) => (
                                <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} ${selectedIds.includes(item.id) ? '!bg-blue-50' : ''}`}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.nomor_permohonan}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(item.tanggal_permohonan).toLocaleDateString()}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.user_pemohon?.name || '-'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.laboratorium?.nama || '-'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 text-xs rounded-full ${statusBadge(item.status_permohonan)}`}>
                                ...
                                <tr>
                                    <td colSpan="7" className="px-6 py-4 text-center text-gray-500">Belum ada permohonan.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                    ...
                    <div className="p-4 border-t border-gray-200">
                        <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-700">
                                Menampilkan {permohonan.from || 0} - {permohonan.to || 0} dari {permohonan.total || 0} data
                            </span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 overflow-y-auto">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                        <div className="p-6 border-b flex justify-between items-center sticky top-0 bg-white z-10">
                            <h2 className="text-xl font-semibold text-gray-800">Buat Permohonan Aset</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                            {/* General Info */}
                             <div className="mb-6">
                                 {!data.laboratorium_id && (
                                     <div className="text-red-500 mb-2">Error: Laboratorium tidak terdeteksi. Pastikan Anda memiliki akses lab.</div>
                                 )}

                                 <label className="block text-sm font-medium text-gray-700 mb-1">Alasan Pengadaan (Umum)</label>
                                 <textarea
                                    value={data.alasan_umum_pengadaan}
                                    onChange={e => setData('alasan_umum_pengadaan', e.target.value)}
                                    className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                    rows="3"
                                    required
                                    placeholder="Jelaskan alasan umum kebutuhan pengadaan aset ini..."
                                 />
                                 {errors.alasan_umum_pengadaan && <div className="text-red-500 text-xs mt-1">{errors.alasan_umum_pengadaan}</div>}
                            </div>
                                    ...
                                    <div key={index} className="bg-gray-50 p-4 rounded-lg mb-4 border border-gray-200 relative">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                             <div>
                                                ...
                                                <input type="text" value={item.nama_barang} onChange={e => updateItem(index, 'nama_barang', e.target.value)} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
                                                {errors[`items.${index}.nama_barang`] && <div className="text-red-500 text-xs mt-1">{errors[`items.${index}.nama_barang`]}</div>}
                                             </div>
                                             <div>
                                                <label className="block text-xs font-medium text-gray-700">Jenis Barang</label>
                                                <input type="text" value={item.jenis_barang} onChange={e => updateItem(index, 'jenis_barang', e.target.value)} placeholder="Contoh: Elektronik" className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                                             </div>
                                             <div>
                                                <label className="block text-xs font-medium text-gray-700">Spesifikasi Teknis</label>
                                                <input type="text" value={item.spesifikasi_teknis} onChange={e => updateItem(index, 'spesifikasi_teknis', e.target.value)} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                                             </div>
                                              <div>
                                                <label className="block text-xs font-medium text-gray-700">Perkiraan Harga (Rp)</label>
                                                <input type="number" value={item.perkiraan_harga} onChange={e => updateItem(index, 'perkiraan_harga', e.target.value)} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                                             </div>
                                             <div className="flex gap-2">
                                                <div className="flex-1">
                                                    <label className="block text-xs font-medium text-gray-700">Jumlah *</label>
                                                    <input type="number" min="1" value={item.jumlah_diminta} onChange={e => updateItem(index, 'jumlah_diminta', parseInt(e.target.value))} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
                                                </div>
                                                <div className="w-1/3">
                                                     <label className="block text-xs font-medium text-gray-700">Satuan</label>
                                                    <input type="text" value={item.satuan} onChange={e => updateItem(index, 'satuan', e.target.value)} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                                                </div>
                                             </div>
                                              <div>
                                                <label className="block text-xs font-medium text-gray-700">Urgensi</label>
                                                <select value={item.urgensi} onChange={e => updateItem(index, 'urgensi', e.target.value)} className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                              ...
                                              <div className="md:col-span-2 lg:col-span-3">
                                                ...
                                                <input type="url" value={item.referensi_url} onChange={e => updateItem(index, 'referensi_url', e.target.value)} placeholder="https://..." className="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                                             </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t">
                                <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">Batal</button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-lg font-medium text-gray-900">Hapus Permohonan Massal</h3>
                        </div>
                        <p className="text-gray-500 mb-6">
                            Apakah Anda yakin ingin menghapus <strong>{selectedIds.length} permohonan</strong> terpilih? Hanya permohonan dengan status "diajukan" yang dapat dihapus.
                        </p>
                        <div className="flex justify-end gap-3">
                            ...
                            <button onClick={executeBulkDelete} className="px-4 py-2 bg-red-600 text-white rounded-md shadow-sm hover:bg-red-700">Hapus {selectedIds.length} Permohonan</button>
                        </div>
                    </div>
                </div>
            )}
