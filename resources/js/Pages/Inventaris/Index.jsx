                    ...
                    filters={[
                        {
                            ...
                            label: "Kategori",
                            control: (
                                <select className="select min-h-11" value={selectedCategory} onChange={onCategoryChange}>
            ...
            <Modal show={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} maxWidth="lg">
                <form onSubmit={handleCreateSubmit} className="flex max-h-[calc(100vh-5rem)] flex-col">
                    ...
                    <div className="space-y-4 overflow-y-auto p-5">
                        ...
                        <FormField label="Nama Barang" error={createForm.errors.nama} hint="Opsional jika sama dengan kategori."><input className="input min-h-11 w-full" value={createForm.data.nama} onChange={(event) => createForm.setData("nama", event.target.value)} /></FormField>
                        <FormField label="Kode Barang" error={createForm.errors.kode_barang} required><input className="input min-h-11 w-full" value={createForm.data.kode_barang} onChange={(event) => createForm.setData("kode_barang", event.target.value)} required /></FormField>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <FormField label="Tanggal Perolehan" error={createForm.errors.tanggal_perolehan}><input type="date" className="input min-h-11 w-full" value={createForm.data.tanggal_perolehan} onChange={(event) => createForm.setData("tanggal_perolehan", event.target.value)} /></FormField>
                            <FormField label="Harga Perolehan (Rp)" error={createForm.errors.harga_perolehan}><input type="number" min="0" className="input min-h-11 w-full" value={createForm.data.harga_perolehan} onChange={(event) => createForm.setData("harga_perolehan", event.target.value)} /></FormField>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <FormField label="Kondisi" error={createForm.errors.keadaan}><select className="select min-h-11 w-full" value={createForm.data.keadaan} onChange={(event) => createForm.setData("keadaan", event.target.value)}><option value="baik">Baik</option><option value="rusak">Rusak</option><option value="hilang">Hilang</option></select></FormField>
                            <FormField label="Status" error={createForm.errors.status}><select className="select min-h-11 w-full" value={createForm.data.status} onChange={(event) => createForm.setData("status", event.target.value)}><option value="tersedia">Tersedia</option><option value="dipinjam">Dipinjam</option></select></FormField>
                        </div>
                        <FormField label="Keterangan" error={createForm.errors.keterangan}><textarea className="textarea min-h-20 w-full" value={createForm.data.keterangan} onChange={(event) => createForm.setData("keterangan", event.target.value)} /></FormField>
                        <FormField label="Foto" error={createForm.errors.foto}><input type="file" accept="image/*" className="file-input file-min-h-11 w-full" onChange={(event) => createForm.setData("foto", event.target.files[0])} /></FormField>
                    </div>
