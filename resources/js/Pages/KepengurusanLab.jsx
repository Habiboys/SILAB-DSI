...
export default function KepengurusanLab({ kepengurusanLab, tahunKepengurusan, flash }) {
    ...
    return <DashboardLayout>
        ...
        <Modal show={modal === 'create'} maxWidth="md" onClose={close}><form onSubmit={submit(createForm, route('kepengurusan-lab.store'), 'Kepengurusan lab berhasil ditambahkan')} className="p-5 sm:p-6"><h2 className="mb-4 text-lg font-semibold">Tambah Kepengurusan Lab</h2><div className="space-y-3"><FormField label="Tahun Kepengurusan" error={createForm.errors.tahun_kepengurusan_id} required><select className={`select min-h-11 w-full ${createForm.errors.tahun_kepengurusan_id ? 'select-error' : ''}`} value={createForm.data.tahun_kepengurusan_id} onChange={(e) => createForm.setData('tahun_kepengurusan_id', e.target.value)} required><option value="">Pilih tahun kepengurusan</option>{availableYears.map((year) => <option key={year.id} value={year.id}>{year.tahun} ({formatTanggal(year.mulai)} sampai {formatTanggal(year.selesai)})</option>)}</select></FormField><FormField label="File SK" hint="PDF, maksimal 5 MB" error={createForm.errors.sk}><input type="file" accept=".pdf" className={`file-input file-min-h-11 w-full ${createForm.errors.sk ? 'file-input-error' : ''}`} onChange={(e) => createForm.setData('sk', e.target.files[0])} /></FormField></div><div className="mt-6 flex justify-end gap-2"><Button variant="ghost" onClick={close}>Batal</Button><Button type="submit" loading={createForm.processing}>Simpan</Button></div></form></Modal>
        <Modal show={modal === 'edit' && !!selected} maxWidth="md" onClose={close}>{selected && <form onSubmit={submit(editForm, route('kepengurusan-lab.update', selected.id), 'SK kepengurusan lab berhasil diperbarui')} className="p-5 sm:p-6"><h2 className="mb-4 text-lg font-semibold">Perbarui SK</h2><FormField label="File SK Baru" hint="PDF, maksimal 5 MB" error={editForm.errors.sk} required><input type="file" accept=".pdf" className={`file-input file-min-h-11 w-full ${editForm.errors.sk ? 'file-input-error' : ''}`} onChange={(e) => editForm.setData('sk', e.target.files[0])} required /></FormField><div className="mt-6 flex justify-end gap-2"><Button variant="ghost" onClick={close}>Batal</Button><Button type="submit" loading={editForm.processing}>Simpan</Button></div></form>}</Modal>
    </DashboardLayout>;
}
