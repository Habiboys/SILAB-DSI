    ...
    return (
        <Modal show={show} onClose={onClose} maxWidth="md">
            ...
            <form onSubmit={onSubmit} className="space-y-4 overflow-y-auto p-4 sm:p-5">
                ...
                <FormField label="Password" error={form.errors.password} hint={editing ? 'Kosongkan jika tidak diubah.' : undefined} required={!editing}><input type="password" className="input min-h-11 w-full" value={form.data.password} onChange={(e) => form.setData('password', e.target.value)} required={!editing} /></FormField>
                <FormField label="Konfirmasi password" required={!editing}><input type="password" className="input min-h-11 w-full" value={form.data.password_confirmation} onChange={(e) => form.setData('password_confirmation', e.target.value)} required={!editing} /></FormField>
                <FormField label="Role" error={form.errors.role} required><select className="select min-h-11 w-full" value={form.data.role} onChange={(e) => form.setData('role', e.target.value)} required>{roles.map((role) => <option key={role.id} value={role.name}>{role.name}</option>)}</select></FormField>
