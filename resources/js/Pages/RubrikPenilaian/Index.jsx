        ...
        <Modal show={open} onClose={() => setOpen(false)} maxWidth="2xl">
            <form onSubmit={submit} className="flex max-h-[calc(100vh-5rem)] flex-col">
                ...
                <div className="space-y-4 overflow-y-auto p-5">
                    <FormField label="Nama Rubrik" error={form.errors.nama_rubrik} required><input className="input min-h-11 w-full" value={form.data.nama_rubrik} onChange={(e) => form.setData('nama_rubrik', e.target.value)} required /></FormField>
                    <FormField label="Deskripsi" error={form.errors.deskripsi}><textarea className="textarea min-h-24 w-full" value={form.data.deskripsi} onChange={(e) => form.setData('deskripsi', e.target.value)} /></FormField>
                    ...
                    {form.data.komponen.map((item, index) => <fieldset key={index} className="rounded-md border border-base-300 p-4"><legend className="px-2 font-medium">Komponen {index + 1}</legend><div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                        <FormField label="Nama Komponen" required><input className="input min-h-11 w-full" value={item.nama_komponen} onChange={(e) => update(index, 'nama_komponen', e.target.value)} required /></FormField>
                        <FormField label="Bobot (%)" required><input type="number" min="0" max="100" step="0.1" className="input min-h-11 w-full" value={item.bobot} onChange={(e) => update(index, 'bobot', Number(e.target.value))} required /></FormField>
                        <FormField label="Deskripsi" className="md:col-span-2"><textarea className="textarea min-h-20 w-full" value={item.deskripsi} onChange={(e) => update(index, 'deskripsi', e.target.value)} /></FormField>
                        <FormField label="Nilai Maksimal" required><input type="number" min="1" max="1000" className="input min-h-11 w-full" value={item.nilai_maksimal} onChange={(e) => update(index, 'nilai_maksimal', Number(e.target.value))} required /></FormField>
