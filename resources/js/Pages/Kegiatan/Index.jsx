...
export default function KegiatanIndex({ kegiatan, filters, can, kepengurusanLabId }) {
    ...
    const columns = useMemo(() => [
        ...
        { key: 'status_approval', header: 'Status', filter: { type: 'select', options: ['diajukan', 'disetujui', 'ditolak'].map((value) => ({ value, label: value[0].toUpperCase() + value.slice(1) })) }, render: (item) => <StatusBadge status={item.status_approval} label={item.status_approval?.[0]?.toUpperCase() + item.status_approval?.slice(1)} /> },
