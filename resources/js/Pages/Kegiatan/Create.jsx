    ...
    <DashboardLayout>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden p-6">
        <div className="border-b pb-4 mb-6">
          <h2 className="text-xl font-semibold text-gray-800">Buat Kegiatan Baru</h2>
          <p className="text-sm text-gray-600 mt-1">Ajukan kegiatan baru untuk program kerja Anda.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              required
            />
            {errors.nama_kegiatan && <div className="text-red-500 text-xs mt-1">{errors.nama_kegiatan}</div>}
          </div>

          {/* Proker */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Program Kerja Terkait</label>
            <select
            ...
            >
              ...
              {proker.map((p) => (
                <option key={p.id} value={p.id}>{p.nama_proker}</option>
              ))}
            </select>
            {errors.proker_id && <div className="text-red-500 text-xs mt-1">{errors.proker_id}</div>}
          </div>
          ...
          <div>
            ...
            ></textarea>
            {errors.deskripsi_kegiatan && <div className="text-red-500 text-xs mt-1">{errors.deskripsi_kegiatan}</div>}
          </div>
