...
export default function KegiatanKalender() {
  ...
  const getEventsForDay = (day) => {
    ...
    return events.filter(e => {
        ...
        return e.start === dateStr || (e.start <= dateStr && e.end > dateStr);
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-[calc(100vh-120px)]">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-gray-50">
            ...
            <div className="flex space-x-2">
                <button onClick={handlePrevMonth} className="p-2 hover:bg-gray-200 rounded-full">
                    &larr;
                </button>
                <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded hover:bg-blue-200">
                    Hari Ini
                </button>
                <button onClick={handleNextMonth} className="p-2 hover:bg-gray-200 rounded-full">
                    &rarr;
                </button>
            </div>
        </div>
        ...
        <div className="grid grid-cols-7 flex-1 overflow-y-auto">
            ...
            {prevMonthDays.map((_, index) => (
                <div key={`prev-${index}`} className="border-b border-r bg-gray-50/50 min-h-[100px]"></div>
            ))}
            ...
            {days.map(day => {
                ...
                return (
                    <div key={day} className={`border-b border-r min-h-[100px] p-2 hover:bg-gray-50 transition-colors ${isToday ? 'bg-blue-50' : ''}`}>
                        ...
                        <div className="space-y-1">
                            {dayEvents.map(event => (
                                <Link 
                                    ...
                                    href={event.url}
                                    title={event.title}
                                    className="block px-2 py-1 text-xs text-white bg-blue-500 rounded truncate hover:bg-blue-600"
                                >
                                    {event.title}
                                </Link>
                            ))}
        ...
        <div className="p-4 border-t flex justify-between items-center bg-gray-50">
             <Link href={route('kegiatan.index')} className="text-sm text-blue-600 hover:underline">
                &larr; Kembali ke Daftar Kegiatan
             </Link>
             <div className="text-xs text-gray-500">
                * Menampilkan kegiatan yang telah disetujui
             </div>
        </div>
      </div>
    </DashboardLayout>
