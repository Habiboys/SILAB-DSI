...
const DataMasterStruktur = ({ struktur, roles }) => {
        ...
        <div className="p-6 flex justify-between items-center border-b">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Kelola data master struktur jabatan dan set role default untuk user
            </p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Struktur</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jabatan Tunggal</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Default Role</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {struktur.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">Belum ada data struktur jabatan</td>
                </tr>
              ) : (
                struktur.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{item.struktur}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item.jabatan_tunggal ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <button onClick={() => openModal(item)} className="text-blue-600 hover:text-blue-900 transition-colors" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
                          <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                        </svg>
                      </button>
                      <button onClick={() => openDeleteModal(item)} className="text-red-600 hover:text-red-900 transition-colors" title="Hapus">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">{editingStruktur ? 'Edit Struktur' : 'Tambah Struktur'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input
                  ...
                  required
                />
                {errors.struktur && <div className="text-red-500 text-xs mt-1">{errors.struktur}</div>}
              </div>
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  onChange={(e) => setData('default_role_id', e.target.value)}
                ...
                {errors.default_role_id && <div className="text-red-500 text-xs mt-1">{errors.default_role_id}</div>}
                <p className="text-xs text-gray-500 mt-1">
                  User yang masuk ke struktur ini akan otomatis mendapatkan role yang dipilih.
                </p>
              </div>
              ...
              <div className="mb-4">
                ...
                {errors.jabatan_tunggal && <div className="text-red-500 text-xs mt-1">{errors.jabatan_tunggal}</div>}
              </div>

              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={closeModal} className="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition">Batal</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-3">
              <button onClick={closeDeleteModal} className="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition">Batal</button>
              <button onClick={handleDelete} disabled={processing} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition disabled:opacity-50">
