        ...
        <DashboardLayout>
            ...
            <Modal show={showModal} maxWidth="2xl" onClose={closeModal}>
                <form onSubmit={handleSubmit} encType="multipart/form-data" className="flex max-h-[calc(100vh-5rem)] flex-col">
                    ...
                    <div className="space-y-4 overflow-y-auto p-5">
                        ...
                        <FormField label="Nama Program Kerja" error={errors.nama_proker} required><input className="input min-h-11 w-full" value={data.nama_proker} onChange={(event) => setData("nama_proker", event.target.value)} placeholder="Contoh: Neo Portofolio x Marketing" required /></FormField>
                        <FormField label="Deskripsi Kegiatan" error={errors.deskripsi} required><textarea className="textarea min-h-20 w-full" value={data.deskripsi} onChange={(event) => setData("deskripsi", event.target.value)} placeholder="Penjelasan singkat kegiatan..." required /></FormField>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <FormField label="Tujuan" error={errors.tujuan}><textarea className="textarea min-h-20 w-full" value={data.tujuan} onChange={(event) => setData("tujuan", event.target.value)} placeholder="Apa yang ingin dicapai..." /></FormField>
                            <FormField label="Sasaran" error={errors.sasaran}><textarea className="textarea min-h-20 w-full" value={data.sasaran} onChange={(event) => setData("sasaran", event.target.value)} placeholder="Target peserta atau pihak terdampak..." /></FormField>
                        </div>
                        <FormField label="Output Kegiatan" error={errors.output_kegiatan}><textarea className="textarea min-h-20 w-full" value={data.output_kegiatan} onChange={(event) => setData("output_kegiatan", event.target.value)} placeholder="Hasil nyata yang dihasilkan..." /></FormField>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <FormField label="Tanggal Mulai" error={errors.tanggal_mulai}><input type="date" className="input min-h-11 w-full" value={data.tanggal_mulai} onChange={(event) => setData("tanggal_mulai", event.target.value)} /></FormField>
                            <FormField label="Tanggal Selesai" error={errors.tanggal_selesai}><input type="date" className="input min-h-11 w-full" value={data.tanggal_selesai} onChange={(event) => setData("tanggal_selesai", event.target.value)} /></FormField>
                        </div>
                        ...
                        <FormField label="Keterangan" error={errors.keterangan}><textarea className="textarea min-h-20 w-full" value={data.keterangan} onChange={(event) => setData("keterangan", event.target.value)} /></FormField>
                        <FormField label="File Dokumen Proker" error={errors.file_proker} hint="PDF, DOC, DOCX. Maksimal 10 MB. Foto dan galeri diunggah di halaman detail setelah proker dibuat."><input type="file" accept=".pdf,.doc,.docx" className="file-input file-min-h-11 w-full" onChange={(event) => setData("file_proker", event.target.files[0])} /></FormField>
                    </div>
            ...
            <Modal show={!!approvingItem} onClose={() => setApprovingItem(null)} maxWidth="md">
                <div className="p-5">
                    ...
                    <FormField label="Catatan" error={approveAction === "reject" && !approveCatatan.trim() ? "Catatan penolakan wajib diisi." : undefined} required={approveAction === "reject"} className="mt-4"><textarea className="textarea min-h-20 w-full" value={approveCatatan} onChange={(event) => setApproveCatatan(event.target.value)} placeholder={approveAction === "approve" ? "Opsional: catatan persetujuan..." : "Jelaskan alasan penolakan..."} /></FormField>
