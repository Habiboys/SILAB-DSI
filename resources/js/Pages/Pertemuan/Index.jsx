...
export default function PertemuanIndex({ praktikum, pertemuan }) {
    ...
    // ── State ──────────────────────────────────────────────────────────
    const [activeParentId, setActiveParentId] = useState(firstParent?.id || null);
    ...
    const orphanedPertemuan = (activeParentId && showSubTabs)
    ...
    const handleDistribusiPertemuan = (e) => {
        ...
        router.post(
            ...
            { pertemuan_ids: distribusiSelected, target_kelas_id: distribusiTargetKelasId },
            {
                preserveScroll: true,
                onSuccess: () => {
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div>
                        ...
                        <h2 className="text-xl font-bold text-gray-800">{praktikum.mata_kuliah}</h2>
                        <p className="text-sm text-gray-500 mt-0.5">Kelola jadwal pertemuan dan absensi per kelas.</p>
                    </div>
                                ...
                                <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                                    <div className="px-1 py-1">
                                        <Menu.Item>
                                            {({ active }) => (
                                                <a
                                                    href={activeKelasId ? route('praktikum.absensi.export-praktikan', { praktikum: praktikum.id, kelasId: activeKelasId }) : '#'}
                                        ...
                                        <Menu.Item>
                                            {({ active }) => (
                                                <a
                                                    href={activeKelasId ? route('praktikum.absensi.export-aslab', { praktikum: praktikum.id, kelasId: activeKelasId }) : '#'}
                                                    className={`${active ? 'bg-blue-600 text-white' : 'text-gray-900'} group flex w-full items-center rounded-md px-2 py-2 text-sm gap-2 ${!activeKelasId ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`}
                                                >
                                                    <Users className="w-4 h-4" />
                                                    Absensi Aslab
                ...
                {showSubTabs && (
                    <div className="px-6 py-2 bg-amber-50 border-b border-amber-100 text-xs text-amber-700 flex items-center gap-2">
                        ...
                        <span>
                            ...
                            Pertemuan dikelola per sub-kelas.
                            {activeSubName && (
                                <span className="ml-1">
                                    Sedang melihat: <strong>{activeSubName}</strong>.
                                </span>
                            )}
                        </span>
                    </div>
                ...
                {showSubTabs && orphanedPertemuan.length > 0 && (
                    <div className="px-6 py-3 bg-orange-50 border-b border-orange-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <div className="flex items-center gap-2 text-sm text-orange-800">
                            ...
                            <span>
                                <strong>{orphanedPertemuan.length} pertemuan</strong> masih di kelas induk{' '}
                                <strong>{activeParent?.nama_kelas}</strong>. Pindahkan ke sub-kelas agar terstruktur.
                            </span>
                        </div>
                        <button
                ...
                <div className="p-6 bg-gray-50/50 min-h-[400px]">
                    {filteredPertemuan.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <h3 className="text-base font-semibold text-gray-700">Belum ada pertemuan</h3>
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Untuk Kelas <span className="text-red-500">*</span>
                                </label>
                                <select
                                    value={data.kelas_id}
                ...
                <div className="p-0">
                        <div className="flex justify-between items-start px-6 py-4 border-b border-gray-100">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                    Pilih pertemuan dari <strong>{activeParent?.nama_kelas}</strong> dan tentukan sub-kelas tujuan.
                                </p>
                            </div>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handleDistribusiPertemuan}>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="block text-sm font-medium text-gray-700">
                                            Pilih Pertemuan ({distribusiSelected.length} dipilih)
                                        </label>
                                        <div className="flex gap-2">
                                            <button type="button" onClick={() => setDistribusiSelected(orphanedPertemuan.map(p => p.id))} className="text-xs text-blue-600 hover:text-blue-800">Pilih Semua</button>
                                            <span className="text-gray-300 text-xs">|</span>
                                            <button type="button" onClick={() => setDistribusiSelected([])} className="text-xs text-gray-500 hover:text-gray-700">Batal Semua</button>
                                        </div>
                                    </div>

                                    <div className="border border-gray-200 rounded-lg divide-y divide-gray-100 max-h-64 overflow-y-auto">
                                        {orphanedPertemuan.map((p) => {
                                            const isChecked = distribusiSelected.includes(p.id);
                                            return (
                                                <label
                                                    key={p.id}
                                                ...
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={isChecked}
                                                        onChange={() => toggleDistribusiItem(p.id)}
                            ...
                            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
                                ...
                                >
                                    {distribusiProcessing ? 'Memindahkan...' : `Pindahkan ${distribusiSelected.length} Pertemuan`}
                                </button>
                            </div>
                        </form>
                </div>
